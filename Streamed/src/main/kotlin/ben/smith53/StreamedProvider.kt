    ...
    suspend fun getUrl(
        ...
        try {
            ...
            if (testResponse.code == 200) {
                // Send m3u8 URL and cookies to Python script
                val pythonEndpoint = "http://localhost:8000/update"
                val payload = mapOf(
                    "m3u8_url" to m3u8Url,
                    "cookies" to cookies.entries.joinToString("; ") { "${it.key}=${it.value}" }
                )
                try {
                    app.post(pythonEndpoint, json = payload, headers = mapOf("Content-Type" to "application/json"))
                    Log.d("StreamedExtractor", "Sent m3u8 URL and cookies to Python script: $m3u8Url")
                } catch (e: Exception) {
                    Log.e("StreamedExtractor", "Failed to send data to Python script: ${e.message}")
                }
            ...
            // Send m3u8 URL and cookies to Python script even if test fails
            val pythonEndpoint = "http://localhost:8000/update"
            val payload = mapOf(
                "m3u8_url" to m3u8Url,
                "cookies" to cookies.entries.joinToString("; ") { "${it.key}=${it.value}" }
