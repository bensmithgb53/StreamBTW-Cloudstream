...
class StreamedProvider : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        Log.d("StreamedProvider", "Match sources for $matchId: ${matchData.matchSources.map { "${it.sourceName} (${it.language})" }}")
        ...
        matchData.matchSources.forEach { source ->
            for (streamNo in 1..maxStreams) {
                ...
                Log.d("StreamedProvider", "Trying stream: $streamUrl (language: ${source.language}, hd: ${source.hd})")
                if (extractor.getUrl(streamUrl, matchId, source.sourceName, streamNo, source.language, source.hd, subtitleCallback, callback)) {
                    success = true
                }
            }
        }
...
class StreamedMediaExtractor {
    ...
    suspend fun getUrl(
        ...
        Log.d("StreamedExtractor", "Extracting: $streamUrl (matchId: $matchId, source: $source, streamNo: $streamNo, language: $language, hd: $hd)")
        ...
        // Map language to English for HD streams
        val streamLanguage = when {
            hd && (language == null || language.contains("Feed", ignoreCase = true)) -> "English"
            else -> language ?: "Unknown"
        }
        val streamQuality = if (hd) Qualities.P1080.value else Qualities.Unknown.value

        // Return the M3U8 URL as the extractor link
        callback.invoke(
            newExtractorLink(
                source = "Streamed",
                name = "$source Stream $streamNo ($streamLanguage)",
                url = finalUrl,
